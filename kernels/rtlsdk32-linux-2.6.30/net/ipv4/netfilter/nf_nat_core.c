...
#if defined (CONFIG_ALPHA_PORT_RESTRICTED_CONE) || defined (CONFIG_ALPHA_RESTRICTED_CONE)
...
Otherwise ,return 0
*/
static int find_appropriate_dest(struct net *net,
							const struct nf_conntrack_tuple *tuple,
							const struct nf_conntrack_tuple *orig_tuple)
{
	...
	hlist_for_each_entry_rcu(nat, n, &net->ipv4.nat_bydest[h], bydest) 
	{
		...
		if (same_dst(ct, tuple)) 
		{
			...
			if ( ! ( t->dst.protonum == orig_tuple->dst.protonum &&
					t->src.u3.ip == orig_tuple->src.u3.ip &&
					t->src.u.all == orig_tuple->src.u.all))
			{
				rcu_read_unlock();
				return 1;
			}
...
#endif
#ifdef CONFIG_ALPHA_RESTRICTED_CONE
...
#else
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
...
TRY_ANOTHER_PORT:
		...
		if ( find_appropriate_dest(net, tuple,orig_tuple) )
...
//Alpha
#ifdef CONFIG_HARDWARE_TURBO
			if (hw_tcpip && hw_tcpip->add_hw_SNAT_entry)
					hw_tcpip->add_hw_SNAT_entry(ct, NF_INET_POST_ROUTING);
#endif			
		}
		else
		{
...
//Alpha
#ifdef CONFIG_HARDWARE_TURBO
			if (hw_tcpip && hw_tcpip->add_hw_DNAT_entry)
				hw_tcpip->add_hw_DNAT_entry(ct, NF_INET_PRE_ROUTING);
#endif
		}
	}
...
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
	//it will have dead loop in find appropriate_conntrack if we add the same
	//conntrack to the list
	//we will add list only when it is src nat and it is not done
	if ( ct->status & IPS_SRC_NAT && !(ct->status & IPS_SRC_NAT_DONE_BIT) )
	{
		unsigned int dsthash = hash_by_src(&new_tuple);
		spin_lock_bh(&nf_nat_lock);
		hlist_add_head_rcu(&nat->bydest,
				   &net->ipv4.nat_bydest[dsthash]);
		spin_unlock_bh(&nf_nat_lock);
	}
	else
		nat->bydest.pprev = NULL;
#endif
...
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
	if (nat->bydest.pprev != NULL)
		hlist_del_rcu(&nat->bydest);
#endif
	spin_unlock_bh(&nf_nat_lock);
}
...
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
	hlist_replace_rcu(&old_nat->bydest, &new_nat->bydest);
#endif
	spin_unlock_bh(&nf_nat_lock);
}
...
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
	net->ipv4.nat_bydest = nf_ct_alloc_hashtable(&nf_nat_htable_size,
						       &net->ipv4.nat_vmalloced, 0);
	if (!net->ipv4.nat_bydest)
		return -ENOMEM;
#endif
...
static void __net_exit nf_nat_net_exit(struct net *net)
{
	...
	nf_ct_free_hashtable(net->ipv4.nat_bysource, net->ipv4.nat_vmalloced,
			     nf_nat_htable_size);
#ifdef CONFIG_ALPHA_PORT_RESTRICTED_CONE
	nf_ct_free_hashtable(net->ipv4.nat_bydest, net->ipv4.nat_vmalloced,
			     nf_nat_htable_size);
#endif
}
