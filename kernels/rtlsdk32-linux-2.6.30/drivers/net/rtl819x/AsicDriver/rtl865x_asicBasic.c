...
int check_8367r_connected(void)
{
	...
	if((smi_write(0x13C2, 0x0249)) == 0)
		if((smi_read(0x1301, &data)) == 0)
			if (data == 0x1010 ||data == 0x1000)
				ret = 1;
...
int32 rtl865x_initAsicFun(uint32 *enable)
{
	...
	*enable = 0;
	...
	{
		if ((icReg == SIG_8197D) && check_8367r_connected())
			*enable |= (HW_NAT | HW_QOS | HW_MCST);
		else
		{
		if((icReg == SIG_8196D) || (icReg == SIG_8197D))
			*enable |= (DUMP_SWITCH | HW_MCST);
...
int32 bsp_swcore_init(uint32 type)
{
	...
	{
		if((type == SIG_819XD) && (icReg != SIG_8196D) && (icReg != SIG_8197D) && (icReg != SIG_8196DT) && (icReg != SIG_8197DT) && !SIG_VOIP(icReg))
		{
			early_console_write("init switch core failed!!!\n", 27);
			ret = FAILED;
		}
		else if((type == SIG_819XDT) && (icReg != SIG_8196DT) && (icReg != SIG_8197DT) && !SIG_VOIP(icReg))
		{
			early_console_write("init switch core failed!!!\n", 27);
			ret = FAILED;
		}
