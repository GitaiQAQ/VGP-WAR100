...
//polo end
...
#endif
	//Alpha
	//polo add for encryption type 7-23
	//Add encrypt type
	//================================
	memset(&iwe, 0, sizeof(iwe));
	iwe.cmd = SIOCGIWENCRYPT;		//here,need to add SIOCGIWENCRYPT value ,on wireless tool code.
	iwe.u.data.length = 20;	//encrypt_str length
	iwe.u.data.flags = 1;	
	
	bss = priv->site_survey.bss_backup[i];	
	
	memset(encrypt_str,0x00,sizeof(encrypt_str));
	memset(authtype_str,0x00,sizeof(authtype_str));	
	
	if((bss.capability&0x10/*cPrivacy*/)==0)
	{
		/*NoEncryption*/
		sprintf(encrypt_str,"NONE");
	}
	else
	{
		if(bss.t_stamp[0]==0)
		...
		else
		{
			...
			{
				/*WPA*/
				idx=sprintf(authtype_str,"WPA");
				if(((bss.t_stamp[0] & 0x0000f000)>>12)==0x4)
				{
					/*WPA-PSK*/
					idx+=sprintf(authtype_str+idx,"PSK");
				}
				wpa_exist=1;
				...
				{
					/*AutoCipher*/
					sprintf(encrypt_str,"%s","TKIP+AES");
				}
				else if(((bss.t_stamp[0] & 0x00000f00)>>8)==0x4)
				{
					/*AES*/
					sprintf(encrypt_str,"%s","AES");
				}
				else if(((bss.t_stamp[0] & 0x00000f00)>>8)==0x1)
				{
					/*TKIP*/
					sprintf(encrypt_str,"%s","TKIP");					
				}
			}
			if(bss.t_stamp[0] & 0xffff0000)
			{
				if(wpa_exist)
					idx += sprintf(authtype_str,"WPA+2");
				/*WPA2*/
				else
					idx += sprintf(authtype_str,"WPA2");
	...
	//printk("This station[%d] encrytion is %s\n",i,encrypt_str);

//	panic_printk("This station encrytion is %s\n",tmp2Buf);	

	current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe, (char *)encrypt_str);//here we will renew define a iwlist struct len,and add on wireless tool too.
	if (current_ev == previous_ev)
#if WIRELESS_EXT >= 17
            return -E2BIG;
#else
	...
	//================================
	memset(&iwe, 0, sizeof(iwe));
	iwe.cmd = SIOCGIAUTHTYPE;		//here,need to add SIOCGIAUTHTYPE value ,on wireless tool code.
	iwe.u.data.length = 20;	//authtype length
	iwe.u.data.flags = 1;	
	//printk("This station[%d] authtype is %s\n",i,authtype_str);	
	current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe, (char *)authtype_str);//here we will renew define a iwlist struct len,and add on wireless tool too.
	if (current_ev == previous_ev)
#if WIRELESS_EXT >= 17
            return -E2BIG;
#else
