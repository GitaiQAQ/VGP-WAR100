...
//polo add 7-26	  
	case SIOCGIWENCRYPT:
	{
		//printf("####enter into SIOCGIWENCRYPT\n");		
		char encryption[20+1];
		memset(encryption, '\0', sizeof(encryption));
		if((event->u.encryption.pointer) && (event->u.encryption.length))
		{
			memcpy(encryption, event->u.encryption.pointer, event->u.encryption.length);
		}
		if(event->u.encryption.flags)
			printf("                    Encrypt:\"%s\"\n", encryption);		
	}
	break;
	...
	case SIOCGIAUTHTYPE:
	{
		//printf("####enter into SIOCGIAUTHTYPE\n");
		char authtype[20+1];
		memset(authtype,'\0',sizeof(authtype));
		if((event->u.authtype.pointer) && (event->u.authtype.length))
		{
			memcpy(authtype, event->u.authtype.pointer, event->u.authtype.length);
		}
		if(event->u.authtype.flags)
			printf("                    Authtype:\"%s\"\n", authtype);			
	}
	break;
//polo end
