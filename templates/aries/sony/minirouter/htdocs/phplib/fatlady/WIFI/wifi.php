...
function check_authtype_encrtype($path, $authtype, $encrtype)
{
	...
	if ($authtype=="OPEN" || $authtype=="SHARED" || $authtype=="WEPAUTO")
	{
		...
		return result("FAILED", $path."/encrtype", "Invalid encryption type.");
	}
	...
	return result("FAILED", $path."/authtype", "Invalid authentication type.");
}
...
function check_wep($p)
{
	...
	if ($err_s > 0)
		return result("FAILED", $p."/wep/size",
				I18N("h","The WEP key should be 5 characters long for 64 bit, and 13 characters long for 128 bit."));
	if ($err_i > 0)
		return result("FAILED", $p."/wep/defkey",
				"The default WEP key index should be between 1 to 4.");
	if ($err_kl > 0)
		return result("FAILED", $p."/wep/key:".$err_kl,
				"The length of WEP key $1 should be $2.",$err_kl, $len);
	if ($err_km > 0)
		return result("FAILED", $p."/wep/ascii",
				I18N("h","Use one-byte alphanumeric characters for the WEP key."));
...
function check_psk($p)
{
	...
	if ($type == "")
	{
		if ($len>=8 && $len<=63)	$type = 1;
						...
						I18N("h","The PSK should be between 8 and 63 one-byte characters."));
	}
	if ($type==0)
	{
		if ($len!=64 || isxdigit($key)!=1)
			return result("FAILED", $p."/psk/key",
				"The PSK should be 64 characters of the hexadecimal number.");
	}
	...
	if ($gtk!="")
	{
		if (isdigit($gtk)!=1)
			return result("FAILED", $p."/rekey/gtk",
					"The value of rekey interval should be a number.");
	}
	...
	if($val!="")
	{
		if (isdigit($val)!=1 || $val > 65535 || $val < 30)
			return result("FAILED", $p."/wpa/groupintv",
					"The value of the group key update interval should be between 30 to 65535.");				
	}
...
function check_wps($p)
{
	...
	if($val==1 && $authtype=="SHARED")
		return result("FAILED", $p."/wps/pin", I18N("h",'"Shared Key" cannot be selected when WPS is enabled.'));
	if ($val!=1) set($p."/wps/enable", 0);

	$pin = query($p."/wps/pin");
	if ($pin!="" && check_pin($pin)!=1)
		return result("FAILED", $p."/wps/pin", I18N("h","This PIN code is invalid."));
...
function check_acl($p)
{
	...
	if ($cnt > $max)
		return result("FAILED", $p."/acl/count", "The ACL rules are full.");
	...
	foreach ($p."/acl/entry")
	{
		$mac = query("mac");
		if (valid_mac($mac)==0)
			return result("FAILED", $p."/acl/entry:".$InDeX, I18N("h","This MAC address is invalid."));
	...
	foreach ($p."/acl/entry")
	{
		...
		if ($err > 0)
			return result("FAILED", $p."/acl/entry:".$InDeX, "Duplicate MAC address.");
	}
...
function check_media($band, $p)
{
		...
		$wlmode == "bgn")
	{
		...
		return result("FAILED", $p."/wlmode",
				"The wireless mode is invalid."." (".$wlmode.")");
	/* channel */
	$v = query($p."/channel");
	...
	if ($v != 0)
	{
		...
		if ($found==0)
			return result("FAILED", $p."/channel",
					"Invalid channel number."." (".$v.")".
					"The valid channel number should be in the following list.".
					" (".$clist.")");
	}
	/* tx rate */
	$valid = 1;
	...
	if ($valid == 0) return result("FAILED", $p."/txrate", "Invalid Tx Rate.");
	/* tx power */
	$v = query($p."/txpower");
...
function fatlady_wifi($prefix, $phyinf)
{
	...
	if ($p=="")
		return result("FAILED", "",
				"The interface is not activated."." (".$phyinf.")");
	...
	if ($val!="STA" && $val!="AP" && $val!="WDS" && $val!="REPEATER" && $val!="APNF" && $val!="APCLI")
	...
	if ($opmode != "APCLI")
	{
	if ($len==0) return result("FAILED", $p."/ssid", I18N("h","The SSID is blank. Enter an SSID."));
	if (isprint($val)!=1)
		return result("FAILED", $p."/ssid", I18N("h","Enter the SSID in one-byte characters."));
	}
	if ($len>32) return result("FAILED", $p."/ssid", "The maximum length of SSID should 32.");
	/* ssidhidden */
	...
	else if	($method=="pin")
	{
		...
		if (check_pin($pin)!=1)
			return result("FAILED",
						...
						I18N("h","This PIN code is invalid."));
	}
	else
		return result("FAILED",
