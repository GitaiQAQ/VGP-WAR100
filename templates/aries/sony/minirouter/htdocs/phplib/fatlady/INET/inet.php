...
function check_inf($entry)
{
	...
	if ($value!="")
	{
		...
		if ($value > 0)
		{
			...
			{
				set_result("FAILED", $entry."/weballow/hostv4ip", I18N("h","This IP address is invalid."));
				return 9;
			}
		}
	}
	...
	if (query($inetp."/ipv4/static")==1)
	{
		...
		foreach ($infp)
		{
			...
			if (query($inetp."/ipv4/mask")=="0")
				return set_result("FAILED", $inetp."/ipv4/mask", I18N("h","This subnet mask is invalid."));
			if (query($v_inetp."/ipv4/mask")=="0")
				return set_result("FAILED", $v_inetp."/ipv4/mask", I18N("h","This subnet mask is invalid."));
				...
				ipv4networkid(query($inetp."/ipv4/ipaddr"), $mask))
			{
				return set_result("FAILED", $inetp."/ipaddr",
					I18N("h","Enter the same network ID as $1 for network ID $2.", $inf, $v_inf));
			}
		}
	}
}
function fatlady_inet($prefix, $inf, $needgw)
{
	...
	if (query($infp."/active") == "1")
	{
		...
		||	 $addrtype == "ppp10") dophp("load", $b."/inet_ppp6.php");
				...
				"Unsupported address type : ".$addrtype); /* internal error, no I18N("h",). */
	}
	else
	{
