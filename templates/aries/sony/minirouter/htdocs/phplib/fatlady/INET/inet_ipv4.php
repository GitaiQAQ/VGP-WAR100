...
function check_ipv4($path, $needgw)
{
	...
	if($ipipmode != "")
	{
		...
		if($ip != "")
		{
			...
			{
				set_result("FAILED", $path."/ipaddr", "The range of B4 IPv4 address is from 192.0.0.2 to 192.0.0.7");
				return;
			}
	...
	if ($static == "1")
	{
		...
		if ($mask=="")
		{
			set_result("FAILED", $path."/mask", I18N("h","Enter the subnet mask."));
			return;
		}
		if ($mask <0 || $mask >32)
		{
			set_result("FAILED", $path."/mask", I18N("h","This subnet mask is invalid."));
			return;
		}
		if ($mask <8)
		{
			set_result("FAILED", $path."/mask", I18N("h","The router does not support subnet masks that are shorter than Class A."));
			return;
		}		
		...
		{
			set_result("FAILED", $path."/gateway", I18N("h","The default gateway cannot have the same address as the IP address."));
			return;
		}
		...
		if ($gw=="")
		{
			...
			{
				set_result("FAILED", $path."/gateway", I18N("h","Enter the default gateway."));
				return;
			}
		}
		else
		{
			...
			{
				set_result("FAILED", $path."/gateway", I18N("h","This default gateway is invalid."));
				return;
			}
			if (ipv4networkid($gw,$mask) != ipv4networkid($ip,$mask))
			{
				set_result("FAILED", $path."/gateway", I18N("h","The default gateway should be in the same network as the IP address."));
				return;
			}
	...
	while ($i < $cnt)
	{
		...
		{
			set_result("FAILED", $path."/dns/entry:".$i, I18N("h","This DNS address is invalid."));
			return;
		}
	...
	if ($mtu!="")
	{
		...
		{
			set_result("FAILED", $path."/mtu",
				I18N("h","The MTU value is too small. Valid values are 576-1500."));
			return;
		}	
		if ($mtu>1500)
		{
			set_result("FAILED", $path."/mtu",I18N("h","The MTU value is too large. Valid values are 576-1500."));
			return;
		}
	}
