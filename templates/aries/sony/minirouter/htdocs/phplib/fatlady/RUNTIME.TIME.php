...
function check_datetime($prefix)
{
	...
	{
		set_result("FAILED", $prefix."/date", "Invalid year"." - ".$year);
		return;
	}
	if (isdigit($month)==0 || $month <= 0 || $month > 12)
	{
		set_result("FAILED", $prefix."/date", "Invalid month");
		return;
	}
	if (isdigit($day)==0 || $day <= 0 || $day > 31)
	{
		set_result("FAILED", $prefix."/date", "Invalid day");
		return;
	}
	...
	{
		set_result("FAILED", $prefix."/time", "Invalid hour");
		return;
	}
	if (isdigit($min)==0 || $min < 0 || $min > 59)
	{
		set_result("FAILED", $prefix."/time", "Invalid minute");
		return;
	}
	if (isdigit($sec)==0 || $sec < 0 || $sec > 59)
	{
		set_result("FAILED", $prefix."/time", "Invalid second");
		return;
	}
	set_result("OK", "", "");
}
