...
foreach ($FATLADY_prefix.$root."/entry")
{
	...
	if ($mac == "")
	{
		$ret = set_result("FAILED", $entry."/mac", I18N("h","There is no MAC address."));
		break;
	}
	if (PHYINF_validmacaddr($mac) != 1)
	{
		$ret = set_result("FAILED", $entry."/mac", I18N("h","This MAC address is invalid."));
		break;
	}
	...
	while ($i <= $count)
	{
		...
		{
			$ret = set_result("FAILED", $entry."/mac", I18N("h","This MAC address has already been set for filtering."));
			break;
		}
		$i++;
	}
