...
while ($i < $cnt)
{
	...
	if ($name == "")
	{
		set_result("FAILED", $FATLADY_prefix."/device/account/entry:".$i."/password", "Login Name cannot be empty.");
		$rlt = "-1";
		break;
	}
	...
	if (query($FATLADY_prefix.$p."/confirmcurrentpassword/enable")==1)
	{
		...
		{
			set_result("FAILED", $FATLADY_prefix.$p."/confirmcurrentpassword/password", "The current password doesn't match.");
			$rlt = "-1";
		}
		break;
	}
