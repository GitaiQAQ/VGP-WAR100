...
{
	$result = "FAIL";
	$reason = "Permission deny. The user is unauthorized.";
}
else
{
	$i = 0;
	while ($i < 2)
	{
		...
		if ($_POST["action"] == "PIN")
		{
			...
			if ($pin == 0)	{ fail(I18N("h","PIN code is incorrect."));	break; }

			$path = XNODE_getpathbytarget("/runtime", "phyinf", "uid", $uid);
			if ($path == "") {fail("Invalid Path!"); break;}
			set($path."/media/wps/enrollee/method", "pin");
			set($path."/media/wps/enrollee/pin", $pin);
			event("WPSPIN");
		}
		/*else if ($_POST["action"] == "PBC")//==20121130 jack removed
		{
			$path = XNODE_getpathbytarget("/runtime", "phyinf", "uid", $uid);
			if ($path == "") {fail(I18N("h","Invalid Path!")); break;}
