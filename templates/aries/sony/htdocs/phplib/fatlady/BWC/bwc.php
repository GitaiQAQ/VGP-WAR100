...
function check_bwcqd($bwcqd, $total_bd)
{
	...
	if( isdigit($bd) == 0 )
	{
		set_result("FAILED", $bwcqd."/bandwidth", I18N("h","Invalid Bandwidth!."));
		return "FAILED";
	}
	if( $bd > $total_bd || $bd < 0 )
	{
...
function check_bwcf($bwcf)
{
	...
	if($mac == "" && $startport == "" && $endport == "")
	{
	    //if($startip == "" || $startip == $lanIp)
	   	if($startip == $lanIp)
	    {
	        set_result("FAILED", $path."/start", I18N("h","Invalid start IP."));
	        return "FAILED";
	    }
	    ...
	    if($endip == $lanIp)
	    {
	        set_result("FAILED", $path."/end", I18N("h","Invalid end IP."));
	        return "FAILED";
	    }
	...
	if($startip == "" && $endip == "" && $startport == "" && $endport == "")
	{
		...
		{
		    set_result("FAILED", $path."/mac", I18N("h","This MAC address is invalid."));
		    return;
		}
	}
	...
	if($mac == "" && $startip == "" && $endip == "")
	{
		if ($startport != "")
		{
			...
			{
				set_result("FAILED", $path."/port/start", I18N("h","Invalid port number"));
				return 0;
			}
			if ($startport<1 || $startport>65535)
			{
				set_result("FAILED", $path."/port/start", I18N("h","Invalid port range"));
				return 0;
			}
		}
...
function check_bwc($prefix)
{
	foreach($prefix."/bwc/entry")
	{
        ...
        {
             set_result("FAILED", $prefix."/entry:".$bwc_index."/autobandwidth", I18N("h","The input autobandwidth is not valid."));
             return "FAILED";
        }
        ...
        {
             set_result("FAILED", $prefix."/entry:".$bwc_index."/bandwidth", I18N("h","The input bandwidth is not digital."));
             return "FAILED";
        }
        $flag = query("flag");
		...
		{
             set_result("FAILED", $prefix."/entry:".$bwc_index."/flag", I18N("h","The input flag is not valid."));
             return "FAILED";
        }
        $bwc_en = query("enable");
        ...
        {
             set_result("FAILED", $prefix."/entry:".$bwc_index."/enable", I18N("h","The input bwc enable is not valid."));
             return "FAILED";
        }
        ...
        foreach($prefix."/bwc/entry:".$bwc_index."/rules/entry")
        {
            ...
            if( $en!="" && $en!="0" && $en!="1")
            {
	             set_result("FAILED", $prefix."/entry:".$bwc_index."/rules/entry:".$InDeX."/enable", I18N("h","The input rules enable is not digital."));
    	         return "FAILED";
			}
