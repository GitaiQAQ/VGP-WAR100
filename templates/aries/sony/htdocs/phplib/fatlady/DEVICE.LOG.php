...
else
{
	...
	$_GLOBALS["FATLADY_message"] = I18N("h","Unsupported log level be assigned");	/* internal error, no i18n. */
}
...
function check_syslog_setting($path, $addrtype, $lan_ip, $mask)
{
	if (query($path."/enable") == "1")
	{
		...
		if ($hostid == "")
		{
			set_result("FAILED", $path."/ipv4/ipaddr", I18N("h","SYSLOG host cannot be empty."));
			return "FAILED";
		}
		if ($hostid <=0)
		{
			set_result("FAILED", $path."/ipv4/ipaddr", I18N("h","SYSLOG host is not a valid host ID."));
			return "FAILED";
		}
		if ($addrtype == "ipv4")
		{
