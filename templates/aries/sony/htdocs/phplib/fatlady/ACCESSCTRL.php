...
{
	set_result("FAILED", $FATLADY_prefix."/acl/accessctrl/count", I18N("h","The rules exceed maximum."));
	$rlt = "-1";
}
else
{
	...
	foreach ($FATLADY_prefix.$root."/entry")
	{
		...
		if($url == "")
		{
			$ret = set_result("FAILED",$entry."/url", I18N("h","No URL for this rule."));
			break;
		}
		...
		if(isdomain($url)!=1)
		{
			$ret = set_result("FAILED",$entry."/url", I18N("h","Invalid URL."));
			break;
		}
		...
		while ($i <= $cnt)
		{
			...
			if ($url == $url_i)
			{
				$ret = set_result("FAILED", $entry."/url", I18N("h","Duplicated URL."));
				break;
			}
			$i++;
		}
