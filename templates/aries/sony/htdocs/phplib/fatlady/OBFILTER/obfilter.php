...
function verify_schedule($uid)
{
	...
	if ($path!="") return "";
	return I18N("h","Incorrect schedule setting.");
}
...
function verify_interface($uid)
{
	...
	if ($path!="") return "";
	return I18N("h","Incorrect interface !")." (".$uid.")";
}
...
function verify_ip_range($ipaddr1, $ipaddr2, $inf)
{
	...
	if($ipaddr1!="")
	{
		if (INET_validv4addr($ipaddr1)!=1)	return I18N("h","Invalid format of the start IP address.");
	}
	if ($ipaddr2!="")
	{
		if (INET_validv4addr($ipaddr2)!=1)	return I18N("h","Invalid format of the end IP address.");
		/* We only allow the range for 255 hosts. */
		$hostid1 = ipv4hostid($ipaddr1, 0);
		$hostid2 = ipv4hostid($ipaddr2, 0);
		$delta = $hostid2-$hostid1;
		if ($delta < 0)				return I18N("h","The end IP address should be greater than the start address.");
		if ($delta > 255)				return I18N("h","The IP address range is too large.");
	}
	$inf0 = cut($inf, 0, "-");
	if ($inf0 == "LAN")
	{
		...
		if($ipaddr1==$lanip)	return I18N("h","The start IP address can't be the same as device IP.");
		if(INET_validv4host($ipaddr1, $lanmask)==0)	return I18N("h","The start IP address is invalid.");
		if(INET_validv4network($ipaddr1, $lanip, $lanmask)==0) 
			return I18N("h","The start IP address and router address should be in the same network subnet.");
		if($ipaddr2!="")
		{
			if($ipaddr2==$lanip)	return I18N("h","The end IP address can't be the same as device IP.");
			if(INET_validv4host($ipaddr2, $lanmask)==0)	return I18N("h","The end IP address is invalid.");
			if(INET_validv4network($ipaddr2, $lanip, $lanmask)==0) 
				return I18N("h","The end IP address and router address should be in the same network subnet.");
		}
	}
	return "";
}
...
function verify_port_range($port1, $port2)
{
	...
	if ($port1=="")					return I18N("h","Require start port for port range.");
	if (isdigit($port1)!="1")		return I18N("h","Invalid start port value.");
	if ($port1<1 || $port1>65535)	return I18N("h","The port is out of the boundary.");
	if ($port2!="")
	{
		if (isdigit($port2)!="1")	return I18N("h","Invalid end port value.");
		if ($port2 > 65535)			return I18N("h","The end port value is too large.");
		if ($port2 < $port1)		return I18N("h","The end port should be greater than the start port.");
	}
	return "";
}
...
function verify_setting($path)
{
	...
	foreach ($path."/entry")
	{
		...
		{
			seterr(I18N("h","Unsupported protocol type."), $path."/entry:".$InDeX."/protocol");
			break;
		}
		...
		{
			seterr(
				I18N("h","The source and destination interfaces should not be the same."),
		...
		if ($err!="")
		{
			$err = I18N("h","Incorrect source IP address.")." ".$err;
		...
		if ($err!="")
		{
			$err = I18N("h","Incorrect destination IP address.")." ".$err;
